...
class TestAddStudentColleges(unittest.TestCase):
    ...
    def test_left_joins_student_data_to_major_metadata_using_major_as_the_join_key(self):
        students = pd.DataFrame(data={'major': ['economics', 'comp sci', 'economics']})
        major_metadata = pd.DataFrame(data={'major': ['economics', 'comp sci'], 'college': ['ksas', 'wse'], 'department': ['pol_sci_econ', 'comp_elec_eng']})
